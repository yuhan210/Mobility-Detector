...
def genFoldsOpt(fold_num, target_length, traceDict, delta, dest_path):
	...
	for activity in traceDict:
		...
		for i in xrange(fold_num):
			...
			cur_total_time = 0
			set_counter = 0
			while(not (cur_total_time < upper_bound and cur_total_time > lower_bound)):
				# start picking the trace
	
				index = round((set_counter % total_set_num)/ total_set_num) * len(sorted_file_pair)
				if cur_total_time < lower_bound and (sorted_file_pair[index]['length'] + cur_total_time) > upper_bound:
					
					for pair in sorted_file_pair:
						if pair['length'] + cur_total_time > lower_bound and pair['length'] + cur_total_time < upper_bound:
							## copy file
							new_file_name = trace['path'].split("/")[-1]
							activity_dir = getActivity(activity)
							print dest_root+"/"+str(i+1)+"/"+activity_dir+"/"+new_file_name
							shutil.copy(trace['path'], dest_root+"/"+str(i+1)+"/"+activity_dir+"/"+ new_file_name)
							##
