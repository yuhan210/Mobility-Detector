...
for seedFolder in seedListing:
	...
	for f in fileListing:
		if f.find("stderr.out") >= 0:
			...
			sampling_rate = int(f.split("_")[0])
			for line in foo.readlines():
				accuracy = 0
				if line.find("Hard match") >= 0:
					accuracy = float(line.split(" ")[3])
					if sampling_rate <= 10:
						 hard_match[sampling_rate -1].append(accuracy)

					elif sampling_rate > 10:
						hard_match[int(sampling_rate/10)+8].append(accuracy)
				
				if line.find("Soft match") >= 0:
					accuracy = float(line.split(" ")[3])
					if sampling_rate <= 10:
						 soft_match[sampling_rate-1].append(accuracy)

					elif sampling_rate > 10:
						soft_match[int(sampling_rate)/10 + 8].append(accuracy)
